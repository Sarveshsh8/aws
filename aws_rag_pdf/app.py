...
# from aws_rag_txt2sql_pdf import TextToSQL  # Ensure this file is named text_to_sql.py or modify accordingly

# # Initialize the TextToSQL class with the CSV file path and FAISS index storage path
# text2sql = TextToSQL("/home/sarveshharikant/EXIMIETAS/SARVESH/AWS/archive/Students data.pdf", "faiss_index_csv")
...
import os
import chainlit as cl
from aws_rag_txt2sql_pdf import TextToSQL  
...
async def handle_message(message: cl.Message):
    ...
    if message.elements:
        ...
        await cl.Message("✅ PDF uploaded successfully! Now ask your question.").send()
        return

    # If no PDF is uploaded yet, ask for one
    if text2sql is None:
        await cl.Message("⚠️ Please upload a PDF first.").send()
        return
